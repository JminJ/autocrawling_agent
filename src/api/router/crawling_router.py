...
async def task_screenshot_split_save(
    webpage_url: str, screenshot_save_base_dir: str
) -> t.Union[str, bool]:
    try:
        ...
        screenshot_save_dir_path = os.path.join(
            screenshot_save_base_dir, webpage_url.replace("/", "-")
        )
        ...
        image_batch_save(
            image_split_batches=screenshot_image_batch, save_dir_path=screenshot_save_dir_path
        )
...
async def autocrawling_operation(input: CrawlingRequest):
    ...
    if not os.path.isdir(screenshot_save_base_dir):
        print("===== screenshot save base dir is not defined, mkdir it =====")
        os.mkdir(screenshot_save_base_dir)
